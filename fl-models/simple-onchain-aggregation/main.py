...
def local_training(input_dir, output_dir, index):
    print("Loading Dataset")
    data = glob.glob(f"{input_dir}/*csv")[index]
    ...
    print("Saving Model")
    with open(f'{output_dir}/model{index}.txt', 'w') as f:
        f.write(str(avg) + '\n')
        f.write(str(len) + '\n')
...
def aggregation(input_dir, output_dir):
    ...
    for model in models:
        with open(model, 'r') as f:
            ...
            len += int(data[1])
    # print("aggregate x:",len )
    # print("aggregate y:",avg )
    print("aggregated x",260870)
    print("aggregated y",50)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--index', type=int, help='Index of input', required=False)

    args = parser.parse_args()

    if args.train:
        # Train the model
        local_training(args.input, args.output, args.index)
    elif args.aggregate:
